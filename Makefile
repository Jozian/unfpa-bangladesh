.PHONY: all base-build base-build-no-cache base-push build build-no-cache down deps-down prod-down network phony-explicit reset shell up deps-up prod-up
...
reset:
	@rm -rf ./.CONTAINER_PRIMERO_CONFIGURED
	@sudo rm -rf ../primero-data/
	@mkdir ../primero-data/

	@mkdir -p ../primero-data/beanstalkd/data/

	@mkdir -p ../primero-data/db/data/
	@mkdir -p ../primero-data/db/etc/
	@mkdir -p ../primero-data/db/log/
	@sudo chown -R 5984:5984 ../primero-data/db/

	@mkdir ../primero-data/letsencrypt/

	@mkdir -p ../primero-data/primero/log/
	@mkdir -p ../primero-data/primero/public/
	@mkdir -p ../primero-data/primero/tmp/
	@sudo chown -R 1000:1000 ../primero-data/primero/

	@mkdir -p ../primero-data/solr/mycores/
	@sudo chown -R 8983:8983 ../primero-data/solr/
